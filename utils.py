...
def score(ball_pos, hoop_pos):
    ...
    if len(x) > 1:
        ...
        # Widen horizontal acceptance window so legit makes aren't missed
        rim_x1 = hoop_pos[-1][0][0] - 0.6 * hoop_pos[-1][2]
        rim_x2 = hoop_pos[-1][0][0] + 0.6 * hoop_pos[-1][2]

        # Check if predicted path crosses the rim area (including rebound zone)
        if rim_x1 < predicted_x < rim_x2:
            return True
        # Check if ball enters rebound zone near the hoop (tolerant buffer)
        hoop_rebound_zone = 20  # pixels
        if rim_x1 - hoop_rebound_zone < predicted_x < rim_x2 + hoop_rebound_zone:
            return True

    # Fallback: if any of the last few ball centers are inside the hoop region, count as make
    # This helps when occlusion or sparse detections break the linear fit.
    if len(ball_pos) > 0 and len(hoop_pos) > 0:
        recent = ball_pos[-6:]  # check last 6 observations
        x_center = hoop_pos[-1][0][0]
        y_center = hoop_pos[-1][0][1]
        w = hoop_pos[-1][2]
        h = hoop_pos[-1][3]
        x1 = x_center - 1.0 * w
        x2 = x_center + 1.0 * w
